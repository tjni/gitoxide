...
pub fn diff<S: Sink>(mut before: &[Token], mut after: &[Token], num_tokens: u32, mut sink: S) -> S::Out {
...
impl Histogram {
    ...
    fn run(
        ...
        loop {
            if before.is_empty() {
                if !after.is_empty() {
                    sink.process_change(before_off..before_off, after_off..after_off + after.len() as u32);
                }
                return;
            } else if after.is_empty() {
                sink.process_change(before_off..before_off + before.len() as u32, after_off..after_off);
                return;
            }
